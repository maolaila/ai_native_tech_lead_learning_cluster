...
def wait_until(action, predicate, timeout: int, label: str):
    ...
    while time.monotonic() < deadline:
        ...
        time.sleep(1)
    raise RuntimeError(
        label + " 在限定时间内未满足；检查 docker compose logs --tail=100 backend"
    )


def main() -> None:
    require(
        urlparse(BASE).hostname in {"localhost", "127.0.0.1", "::1"},
        "Smoke 仅接受本机地址，禁止用于生产",
    )
    wait_until(
        lambda: request("GET", "/actuator/health/readiness"),
        lambda data: data.get("status") == "UP",
        180,
        "后端健康检查",
    )
    run = uuid.uuid4().hex
    ...
    request(
        "POST",
        "/api/auth/register",
        {"email": email, "displayName": "Smoke learner", "password": password},
    )
    buyer = request("POST", "/api/auth/login", {"email": email, "password": password})[
        "accessToken"
    ]
    admin = request(
        "POST",
    ...
    require(
        request("GET", f"/api/products/{product['id']}")["status"] == "PUBLISHED",
        "商品没有上架",
    )
    ...
    require(
        order["totalAmount"] == 200 and order["currency"] == "CNY", "服务端金额不正确"
    )
    payment_path = f"/api/payments/orders/{order['id']}"
    payment = request(
        "POST", payment_path, {"paymentToken": "success"}, buyer, "pay-" + run
    )
    require(payment["status"] == "SUCCEEDED", "模拟支付没有成功")
    require(
        request("GET", f"/api/orders/{order['id']}", token=buyer)["status"] == "PAID",
        "支付返回成功但订单未落库为 PAID",
    )
    require(
        request("POST", payment_path, {"paymentToken": "success"}, buyer, "pay-" + run)[
            "paymentId"
        ]
        == payment["paymentId"],
        "支付重试没有复用原结果",
    ...
    wait_until(
        lambda: request("GET", "/api/notifications", token=buyer),
        lambda rows: any(order["id"] in row["message"] for row in rows),
        60,
        "付款通知（真实 RabbitMQ 消费）",
    )
    ...
    require(
        request("POST", refund_path, token=buyer, key="refund-" + run)["refundId"]
        == refund["refundId"],
        "退款重试没有复用原结果",
    )
    require(
        request("GET", f"/api/orders/{order['id']}", token=buyer)["status"]
        == "REFUNDED",
        "退款未落库",
    )
    print(
