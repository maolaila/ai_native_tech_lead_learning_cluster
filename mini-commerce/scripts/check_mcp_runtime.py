...
with tooling.connect_readonly(os.environ["DATABASE_READONLY_URL"]) as connection:
    row = connection.execute(
        "select current_user, has_table_privilege(current_user,'orders','SELECT'), has_table_privilege(current_user,'orders','INSERT'), has_table_privilege(current_user,'users','SELECT')"
    ).fetchone()
