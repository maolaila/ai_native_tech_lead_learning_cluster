 ...
 *
 * <p><strong>作用：</strong>把“未认证”和“已认证但无权限”转换成结构稳定的 401/403 JSON 响应，
 * 并附带 traceId，方便前端分支处理和日志关联。</p>
 *
 * <p><strong>为什么不能只依赖默认响应：</strong>默认安全错误可能返回 HTML，且与业务接口的错误结构不同；
 * 统一响应能让 API、监控和自动测试使用稳定语义，同时避免把异常堆栈暴露给客户端。</p>
 *
 * <p><strong>对应文档：</strong>
 * {@code 02_backend_spring/04_API设计_校验_异常与错误码.md}、
 * {@code 05_auth_security/02_RBAC与对象级权限.md}、
 * {@code 10_observability/01_结构化日志与关联ID.md}。</p>
 */
@Component
