 ...
 *
 * <p><strong>作用：</strong>接收商品查询和管理请求，触发 DTO 校验，并把调用转交给
 * {@link ProductService}。Controller 不直接访问 Repository，也不决定缓存或上下架规则。</p>
 *
 * <p><strong>为什么这样分层：</strong>HTTP 路由、分页参数和状态码属于传输层；商品可售性、缓存失效和审计属于应用用例。
 * 把两者分开后，业务可以脱离 Web 容器测试，也不会因接口形式变化而污染持久化代码。</p>
 *
 ...
 * {@code 06_redis/02_CacheAside_TTL与失效.md}。</p>
 */
@RestController
