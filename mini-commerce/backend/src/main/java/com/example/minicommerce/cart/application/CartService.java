 ...
 *
 * <p><strong>为什么：</strong>同一用户的修改共用事务级数据库锁，首次创建购物车也受到保护。
 * 只锁已有的购物车行不够，因为第一次加入时还没有行可锁；唯一约束继续作为最后防线。
 *
 * <p><strong>对应文档：</strong>{@code 00_start/02_长期项目_Mini_Commerce.md}、
 * {@code 04_database_postgresql/05_并发_锁与库存超卖.md}。
 */
@Service
public class CartService {
    ...
    public CartService(CartRepository carts, CartItemRepository items,
                       ProductService products, IdempotencyLock locks) {
        this.carts = carts;
        this.items = items;
    ...
    public CartView put(Long userId, Long productId, int quantity) {
        ...
        CartEntity cart = carts.findByUserId(userId)
                .orElseGet(() -> carts.save(new CartEntity(userId)));
        CartItemEntity item = items.findByCartIdAndProductId(cart.getId(), productId)
    ...
    public void remove(Long userId, Long productId) {
        lockCart(userId);
        CartEntity cart = carts.findByUserId(userId)
                .orElseThrow(() -> new BusinessException(ErrorCode.CART_ITEM_NOT_FOUND, "购物车为空"));
        CartItemEntity item = items.findByCartIdAndProductId(cart.getId(), productId)
                .orElseThrow(() -> new BusinessException(ErrorCode.CART_ITEM_NOT_FOUND, "购物车项不存在"));
        items.delete(item);
    }
    ...
    private CartView view(CartEntity cart) {
        return new CartView(cart.getId(), items.findByCartIdOrderById(cart.getId()).stream()
                .map(item -> new CartLine(item.getProductId(), item.getQuantity())).toList());
    }
