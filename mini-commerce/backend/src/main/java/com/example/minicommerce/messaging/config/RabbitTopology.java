 ...
 *
 * <p><strong>大白话：</strong>Producer 先把消息交给 Exchange；Exchange 根据 Routing Key 和 Binding 规则，把消息放进一个或多个
 * Queue； Consumer 再从各自 Queue 读取。
 *
 * <p><strong>为什么通知和积分使用两个 Queue：</strong>同一个“订单已支付”事件需要让通知和积分都处理。 两个独立 Queue 会各保存一份消息；如果只是同一个 Queue
 * 配两个 Consumer，它们会竞争消息，通常只有一个拿到。
 *
 * <p><strong>对应文档：</strong> {@code 07_rabbitmq/02_Exchange_Queue_Routing.md}、 {@code
 * 07_rabbitmq/03_Confirm_Ack_Retry_DLQ.md}、 {@code
...
public class RabbitTopology {
    ...
    @Bean
    Binding notificationBinding(Queue notificationQueue, TopicExchange commerceExchange) {
        return BindingBuilder.bind(notificationQueue).to(commerceExchange).with("order.paid.v1");
    }

    /** order.paid.v1 也路由到积分队列，所以通知和积分都会收到。 */
    @Bean
    Binding pointsBinding(Queue pointsQueue, TopicExchange commerceExchange) {
        return BindingBuilder.bind(pointsQueue).to(commerceExchange).with("order.paid.v1");
    }

    /** 商品变化事件只路由到缓存失效队列。 */
    @Bean
    Binding cacheBinding(Queue cacheQueue, TopicExchange commerceExchange) {
        return BindingBuilder.bind(cacheQueue).to(commerceExchange).with("product.changed.v1");
    }

    @Bean
    Binding notificationDead(Queue notificationDlq, DirectExchange deadLetterExchange) {
    ...
    Binding pointsDead(Queue pointsDlq, DirectExchange deadLetterExchange) {
        return BindingBuilder.bind(pointsDlq).to(deadLetterExchange).with(POINTS_Q + ".dead");
    }
