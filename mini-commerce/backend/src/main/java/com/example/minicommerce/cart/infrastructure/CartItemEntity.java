...
@Entity
// @Table：指定表名，并声明“同一购物车 + 同一商品不能重复”的数据库唯一约束。
@Table(
        name = "cart_items",
        uniqueConstraints =
                @UniqueConstraint(
...
public class CartItemEntity extends BaseEntity {
    ...
    // @Id：这是主键，用来唯一找到一条购物车明细。
    @Id
    // @GeneratedValue：新增记录时由数据库自增生成 ID，Java 代码不需要自己计算。
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long id;

    // nullable = false：数据库不允许 cart_id 为空。
    ...
    /** JPA 通过反射还原实体时需要无参构造器，因此保留为 {@code protected}，避免普通业务代码创建字段不完整的对象。 */
     ...
     * 修改购物车数量。
     *
     * <p><strong>为什么不公开 {@code setQuantity}：</strong>有业务含义的方法能够集中保护“不允许零或负数数量”的规则，
