 ...
 *
 * <p><strong>大白话：</strong>业务事务先把事件写进一张“待寄信清单”。这个后台组件定期领取清单中的事件，发送给 RabbitMQ， 收到 Broker
 * 确认后再把事件标记为已发布。
 *
 * <p><strong>为什么不能发送后立刻假设成功：</strong>网络可能中断，所以需要 Publisher Confirm。 即使 RabbitMQ
 * 已收到消息，程序也可能在标记数据库前宕机，因此同一事件仍可能再次发送，Consumer 必须幂等。
 *
