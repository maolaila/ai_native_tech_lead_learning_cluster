...
/**
 * 业务安全回归：测试最终数据库事实、重复请求和真正的并发，而不只看返回值。
...
class BusinessSafetyIT extends AbstractPostgresIT {
    ...
    private OrderResponse create(String key) {
        return createOrders.create(buyer.id(), key,
    ...
    void idempotencyConflictRollsBackWithoutTakingMoreStock() {
        ...
        assertThatThrownBy(() -> createOrders.create(buyer.id(), "fingerprint", changed)).isInstanceOf(BusinessException.class);
    ...
    void concurrentSameKeyCreatesOneOrder() throws Exception {
        try (var pool = java.util.concurrent.Executors.newFixedThreadPool(2)) {
            ...
            java.util.concurrent.Callable<UUID> action = () -> { start.await(); return create("concurrent").id(); };
    ...
    void unknownPaymentCannotBeRetriedWithANewKeyOrCancelled() {
        ...
        assertThat(paymentFlow.pay(order.id(), buyer, "unknown-key", "timeout").paymentId()).isEqualTo(first.paymentId());
        assertThatThrownBy(() -> paymentFlow.pay(order.id(), buyer, "new-key", "success")).isInstanceOf(BusinessException.class);
    ...
    void declinedPaymentAllowsCancellation() {
        ...
        assertThat(paymentFlow.pay(order.id(), buyer, "decline-key", "decline").status()).isEqualTo("DECLINED");
    ...
    void refundReplayStillChecksOwnership() {
        ...
        var other = users.save(new UserEntity(UUID.randomUUID() + "@example.com", "other", "hash", UserRole.USER));
        assertThatThrownBy(() -> refunds.refund(payment.paymentId(), UserPrincipal.from(other), "private-key")).isInstanceOf(BusinessException.class);
    }
    ...
    void duplicateConsumersDoNotDuplicateSideEffects() throws Exception {
        ...
        String payload = jdbc.queryForObject("select payload from outbox_events where aggregate_id=? and event_type='order.paid.v1'", String.class, order.id().toString());
        consumers.notifyUser(payload);
        consumers.notifyUser(payload);
        consumers.addPoints(payload);
        consumers.addPoints(payload);
        assertThat(jdbc.queryForObject("select count(*) from notifications where user_id=?", Integer.class, buyer.id())).isEqualTo(1);
        assertThat(jdbc.queryForObject("select count(*) from points_ledger where order_id=?", Integer.class, order.id())).isEqualTo(1);
        assertThat(jdbc.queryForObject("select points from points_ledger where order_id=?", Long.class, order.id())).isEqualTo(2L);
    }

    @Test
    void failedConsumerRollsBackDedupMarker() throws Exception {
        UUID eventId = UUID.randomUUID();
        var bad = new com.example.minicommerce.messaging.application.EventEnvelope(eventId, "order.paid.v1", 1,
                java.time.Instant.now(), "test", "ORDER", UUID.randomUUID().toString(), null, json.createObjectNode());
        String raw = json.writeValueAsString(bad);
        assertThatThrownBy(() -> consumers.notifyUser(raw)).isInstanceOf(Exception.class);
        assertThat(jdbc.queryForObject("select count(*) from processed_messages where event_id=?", Integer.class, eventId)).isZero();
    }

    @Test
    void staleOutboxWorkerCannotOverwriteNewLease() {
        var order = create("lease-order");
        UUID eventId = jdbc.queryForObject("select event_id from outbox_events where aggregate_id=?", UUID.class, order.id().toString());
        var a = outboxJobs.claim("worker-a", 1000, java.time.Duration.ofSeconds(30)).stream().filter(e -> e.eventId().equals(eventId)).findFirst().orElseThrow();
        jdbc.update("update outbox_events set locked_until=now()-interval '1 second' where event_id=?", eventId);
        var b = outboxJobs.claim("worker-b", 1000, java.time.Duration.ofSeconds(30)).stream().filter(e -> e.eventId().equals(eventId)).findFirst().orElseThrow();
    ...
    void refreshTokenCanOnlyBeRotatedOnceConcurrently() throws Exception {
        var account = authentication.register(new com.example.minicommerce.identity.api.AuthDtos.RegisterRequest(UUID.randomUUID() + "@example.com", "refresh user", "LongPassword123!"));
        var request = new com.example.minicommerce.identity.api.AuthDtos.RefreshRequest(account.refreshToken());
        try (var pool = java.util.concurrent.Executors.newFixedThreadPool(2)) {
            var start = new java.util.concurrent.CountDownLatch(1);
            java.util.concurrent.Callable<Boolean> action = () -> {
                start.await();
                try { authentication.refresh(request); return true; }
    ...
    void bcryptByteLimitIsAValidationError() {
        var request = new com.example.minicommerce.identity.api.AuthDtos.RegisterRequest(UUID.randomUUID() + "@example.com", "name", "密".repeat(25));
        assertThatThrownBy(() -> authentication.register(request)).isInstanceOf(BusinessException.class);
    }
    ...
    void protectedApiAndManagementRoutesEnforcePermissions() throws Exception {
        ...
        http.perform(get("/api/orders/" + order.id()).with(user(UserPrincipal.from(other)))).andExpect(status().isForbidden());
        http.perform(get("/actuator/metrics").with(user(buyer))).andExpect(status().isForbidden());
        http.perform(put("/api/orders/" + order.id()).with(user(buyer))).andExpect(status().isMethodNotAllowed());
    }
}
