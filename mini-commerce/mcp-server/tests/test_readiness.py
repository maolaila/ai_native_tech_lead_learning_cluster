...
def test_nested_redaction_is_structural_and_keeps_valid_json():
    value = {"password": "plain", "nested": [{"access_token": "abc"}],
             "text": "AWS=" + "AKIA" + "A" * 16, "normal": "quoted \" data"}
    result = redact(value)
...
@pytest.mark.parametrize("sql", ["EXPLAIN SELECT 1", "EXPLAIN ANALYZE SELECT 1", "select * into new_table from orders", "select 1 -- comment", "with x as (delete from orders returning *) select * from x"])
...
def test_runbook_escape_blocked(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError): tooling.read_runbook("../other.md")
