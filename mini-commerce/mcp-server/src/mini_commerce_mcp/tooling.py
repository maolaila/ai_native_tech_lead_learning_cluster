...
SUITES = {"backend-unit": ["mvn", "-B", "test"],
...
def audit(tool: str, args: dict[str, Any], status: str, duration_ms: int) -> None:
    ...
    row = {"observedAt": datetime.now(timezone.utc).isoformat(), "actor": "local-learning-client",
           "tool": tool, "arguments": redact(args), "status": status, "durationMs": duration_ms}
    ...
    def decorate(function):
        ...
        def wrapped(*args, **kwargs):
            ...
            try:
                ...
                audit(tool, {"args": args, "kwargs": kwargs}, status,
...
def search_docs(query: str, limit: int = 8) -> dict[str, Any]:
    ...
    for path in candidates[:2500]:
        ...
        if score:
            ...
            results.append((score, {"source": str(path.relative_to(ROOT)), "score": score,
                                   "excerpt": untrusted_excerpt(excerpt)}))
    data = [item for _, item in sorted(results, key=lambda row: (-row[0], row[1]["source"]))[:limit]]
    return {"status": "ok", "data": data, "truncated": len(results) > limit or len(candidates) > 2500,
...
def read_runbook(relative_path: str) -> dict[str, Any]:
    ...
    return {"status": "ok", "source": str(path.relative_to(ROOT)),
            "data": untrusted_excerpt(text), "truncated": len(text) > 2000}
...
def database_schema() -> dict[str, Any]:
    ...
    if not url:
        folder = safe_resolve(ROOT, "mini-commerce/backend/src/main/resources/db/migration")
        files = sorted(folder.glob("V*.sql"))
        migrations = [{"source": str(path.relative_to(ROOT)),
                       "ddl": bounded_read(safe_resolve(ROOT, str(path.relative_to(ROOT))))}
                      for path in files]
        result = {"mode": "migration-history", "migrations": migrations,
                  "note": "这是有序迁移历史，不是已经连接数据库验证的最终 Schema。"}
        truncated = False
    else:
        with connect_readonly(url) as connection:
            rows = connection.execute("select table_name,column_name,data_type,is_nullable from information_schema.columns where table_schema='public' order by table_name,ordinal_position limit 2001").fetchall()
        result = {"mode": "live-readonly", "columns": [
            {"table": r[0], "column": r[1], "type": r[2], "nullable": r[3]} for r in rows[:2000]]}
...
def explain_readonly(sql: str) -> dict[str, Any]:
    ...
    url = os.getenv("DATABASE_READONLY_URL")
    if not url:
        raise RuntimeError("DEPENDENCY_UNAVAILABLE: DATABASE_READONLY_URL is not configured")
    with connect_readonly(url) as connection:
        plan = connection.execute("EXPLAIN (FORMAT JSON) " + safe).fetchone()[0]
    return {"status": "ok", "data": redact(plan), "analyze": False,
            "note": "不执行 EXPLAIN ANALYZE；仍需真正的只读数据库角色，SQL 关键词检查不是安全沙箱。"}


def execution_enabled() -> bool:
    return (os.getenv("MCP_ENABLE_TEST_EXECUTION") == "true"
            and os.getenv("MCP_TRANSPORT", "stdio") == "stdio" and os.name == "posix")
...
def run_suite(name: str) -> dict[str, Any]:
    ...
    if not execution_enabled():
        raise ValueError("PERMISSION_DENIED: code execution disabled; trusted local stdio opt-in required")
    cwd = safe_resolve(ROOT, "mini-commerce/" + ("mcp-server" if name == "mcp" else "backend"))
    ...
    with tempfile.TemporaryFile() as output:
        process = subprocess.Popen(SUITES[name], cwd=cwd, stdout=output, stderr=subprocess.STDOUT,
                                   env=environment, shell=False, start_new_session=True)
        try:
    ...
    return {"status": "passed" if exit_code == 0 else "failed", "exitCode": exit_code,
            "command": SUITES[name], "output": redact(text), "truncated": size > 20000}
