...
def validate_output(output: Path, repo: Path = REPO_ROOT) -> Path:
    ...
    if output.exists() and (not output.is_dir() or any(output.iterdir())):
        if not marker.is_file() or marker.read_text(encoding="utf-8") != str(repo):
            raise ValueError(
                "拒绝清理非本工具拥有的非空目录；请使用新的 --output 路径，不要删除未知文件"
            )
    return marker
...
def runtime_config(docs: Path, output: Path) -> str:
    ...
    for name, path in (("docs_dir", docs), ("site_dir", output)):
        text, count = re.subn(
            rf"(?m)^{name}:\s*.*$",
            lambda _: f"{name}: {json.dumps(str(path))}",
            text,
            count=1,
        )
        if count != 1:
            raise ValueError(f"mkdocs.yml 必须包含一行 {name}")
    return text


def main() -> int:
    parser = argparse.ArgumentParser(
        description="隔离快照构建或预览学习文档；新增文件先 git add，修改后重启预览"
    )
