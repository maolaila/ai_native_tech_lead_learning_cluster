...
def main() -> None:
    ...
    spec = importlib.util.spec_from_file_location(
        "mapping_builder", ROOT / "tools/generate_complete_mini_commerce_v2.py"
    )
    ...
    mapping["scope"] = (
        "suggested reading targets, not proof that every chapter requirement is implemented"
    )
    ...
    note_path.write_text(
        note_path.read_text(encoding="utf-8").replace(
            ...
            "代码目标是建议阅读的相关位置，不表示该章节的全部验收目标已实现。",
        ),
        encoding="utf-8",
    )
    for item in mapping["entries"]:
        if not (ROOT / item["source"]).is_file():
            raise ValueError(f"来源不存在：{item['source']}")
        for target in item["codeTargets"]:
            if not (ROOT / target).exists():
                raise ValueError(f"代码目标不存在：{target}")
    ...
    files = sorted(
        {
            ROOT / name
            for name in tracked
            if name.startswith("mini-commerce/")
            and (ROOT / name).is_file()
            and not name.endswith("DELIVERY-MANIFEST.json")
        }
    )
    manifest = {
        "scope": "source inventory, not test results",
        "fileCount": len(files),
        ...
        "migrations": sorted(
            p.name
            for p in (project / "backend/src/main/resources/db/migration").glob(
                "V*.sql"
            )
        ),
        "sha256": {
