...
spec = importlib.util.spec_from_file_location("builder", Path(__file__).parents[1] / "build_docs_site.py")
...
class OutputSafetyTest(unittest.TestCase):
    def test_rejects_repository_and_ancestors(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            for unsafe in [repo, repo / "site", repo.parent, Path(repo.anchor)]:
                with self.assertRaises(ValueError): builder.validate_output(unsafe, repo)

    def test_rejects_unowned_nonempty_folder(self):
        with tempfile.TemporaryDirectory() as folder:
            repo, output = Path(folder) / "repo", Path(folder) / "existing"
            repo.mkdir(); output.mkdir(); (output / "important.txt").write_text("keep")
            with self.assertRaises(ValueError): builder.validate_output(output, repo)
    ...
    def test_allows_new_or_owned_output(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            output.mkdir(); (output / "index.html").write_text("site")
    ...
    def test_resolves_symlink_before_check(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            repo.mkdir(); alias.symlink_to(repo, target_is_directory=True)
            with self.assertRaises(ValueError): builder.validate_output(alias, repo)
