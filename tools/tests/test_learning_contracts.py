...
spec = importlib.util.spec_from_file_location(
    "scanner", ROOT / "tools/java_source_scan.py"
)
...
class LearningContractTest(unittest.TestCase):
    def test_annotation_scanner_ignores_comments_literals_and_preserves_lines(self):
        ...
        self.assertEqual(
            list(scanner.annotations(source)), [("NotNull", 4), ("Valid", 4)]
        )
    ...
    def test_all_actual_annotations_are_explained_locally(self):
        dictionary = (
            ROOT / "mini-commerce/docs/SPRING-JAVA-ANNOTATIONS.md"
        ).read_text()
        names = set()
        ...
        self.assertEqual(
            sorted(name for name in names if "@" + name not in dictionary), []
        )
    ...
    def test_permission_probe_names_exist_in_real_migrations(self):
        ...
        known_tables = set(
            re.findall(r"create\s+table\s+(?:if\s+not\s+exists\s+)?(\w+)", sql, re.I)
        )
